...
def calculate_split_point(dataset):
    ...
    def dataset_split(dataset , value , column):
        ...
        for row in dataset:
            if column not in category_list and row[column] < value:
                left.append(row)
            elif column in category_list and row[column] == value:
                left.append(row)
            else:
                right.append(row)
...
def decision_tree_build(node):
    ...
    if len(set(left[:,-1])) == 1:
        ...
        node['left'] = decision_tree_build(calculate_split_point(left))
    if len(set(right[:,-1])) == 1:
        node['right'] = terminal_nodes([], right)
    else:
        node['right'] = decision_tree_build(calculate_split_point(right))
    return node
...
for iteration in range(10):
    ...
    root = {}
    root = calculate_split_point(train_dataset)
    root = decision_tree_build(root)
    for ele in test_dataset:
        predicted_values.append(make_prediction(root , ele))
